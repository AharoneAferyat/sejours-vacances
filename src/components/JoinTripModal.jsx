  ...
  useEffect(() => {
    ...
    validateShareCode(shareCode).then(result => {
      if (result && result.valid) {
        ...
        setStep('error')
        setError(result?.error || 'Ce lien d\'invitation est invalide ou a expiré.')
      }
    }).catch(() => {
      setStep('error')
        ...
        {step === 'form' && (
          <>
            ...
            <div style={{ textAlign: 'left', marginBottom: '.75rem' }}>
              <label style={{ display: 'block', fontSize: '.72rem', fontWeight: 600, color: '#888', marginBottom: '.3rem', textTransform: 'uppercase', letterSpacing: '.06em' }}>
                Ton prénom *
              </label>
              <input
                ...
                placeholder="ex: Marc"
                autoFocus
                style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid #e0e0e0', fontSize: '.9rem', fontFamily: 'inherit', background: '#f5f4f0', outline: 'none', boxSizing: 'border-box' }}
              />
            </div>
